...
export default function ResumeEditor({ resume, onReset }: Props) {
  ...
  async function handleDownload() {
    ...
    el.innerHTML = buildResumeHTML(data);
  ...
  return (
    <div style={s.wrap} className="animate-fadeIn">
      ...
      <div style={{ display: "flex", gap: spacing.md, marginTop: spacing.xl }}>
        <button style={s.backBtn} onClick={onReset}>← Start Over</button>
        <button style={{ ...s.downloadBtn, opacity: downloading ? 0.6 : 1 }} onClick={handleDownload} disabled={downloading}>
